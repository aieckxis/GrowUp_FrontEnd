...
/* --- CONFIGURATION & TYPES --- */
// ✅ FIXED: Removed /video_feed from base URL to avoid double path
...
const fetchCameraSettingsFromAPI = async (): Promise<{ settings: CameraSettingsState; isRecording: boolean }> => {
    try {
        ...
        console.error("API ERROR: Failed to fetch camera settings:", error)
...
const saveCameraSettingsToAPI = async (settings: CameraSettingsState): Promise<boolean> => {
    try {
        ...
        console.error("API ERROR: Failed to save camera settings:", error)
        return false
    }
}
...
const toggleRecordingAPI = async (shouldRecord: boolean): Promise<boolean> => {
    try {
        ...
        console.error("API ERROR: Failed to toggle recording:", error)
        return false
    }
}
...
const captureSnapshotAPI = async (): Promise<{ success: boolean; snapshot?: Snapshot }> => {
    try {
        ...
        console.error("API ERROR: Failed to capture snapshot:", error)
...
const deleteSnapshotAPI = async (snapshotId: number): Promise<boolean> => {
    try {
        ...
        console.error(`API ERROR: Failed to delete snapshot ${snapshotId}:`, error)
        return false
    }
}

// ✅ NEW: Fetch real plant detections from Pi
...
// ✅ NEW: Fetch real snapshots gallery from Pi
...
const useCameraSettings = (showToast: (message: string, color: 'success' | 'info' | 'warning' | 'default' | 'error') => void) => {
    ...
    const handleSave = async (): Promise<boolean> => {
        ...
        if (success) { setHasChanges(false); showToast("✅ Camera settings saved to Raspberry Pi!", "success") }
        else showToast("❌ Failed to save settings to Raspi.", "error")
        return success
    }
    ...
    const handleToggleRecord = async (shouldRecord: boolean): Promise<boolean> => {
        ...
        if (success) {
            ...
            showToast(shouldRecord ? "🎥 Recording command sent to Raspi." : "⏹️ Stop recording command sent. File is processing...", shouldRecord ? "info" : "success")
        } else showToast("❌ Failed to send recording command to Raspi.", "error")
        return success
    }
...
const Toast: React.FC<ToastProps> = ({ message, visible, color, onClose }) => {
    ...
    return (
        <div className={`fixed bottom-4 left-1/2 -translate-x-1/2 px-6 py-3 rounded-xl shadow-2xl z-[100] flex items-center space-x-3 text-white ${colorMap[color]}`}>
...
// ✅ FIXED: usePathname inside Suspense boundary
const BottomNavigationInner = () => {
    const pathname = usePathname() || "/camera"
    const tabs = [
...
export default function App() {
    ...
    // ✅ NEW: Real data states (replaces PLANT_DETECTIONS & GALLERY_SNAPSHOTS_MOCK)
    ...
    // ✅ NEW: Fetch snapshots on mount
    useEffect(() => {
        let mounted = true
        fetchSnapshotsAPI().then(data => {
            if (mounted) { setGallerySnapshots(data); setSnapshotsLoading(false) }
        })
        return () => { mounted = false }
    }, [])

    // ✅ NEW: Refresh snapshots when gallery opens
    useEffect(() => {
        if (showGallery) {
    ...
    const handleRecord = async () => { await handleToggleRecord(!isRecording) }
    ...
    const handleZoomToggle = () => {
        ...
        showToast(!showZoomControls ? "🔍 Zoom controls enabled." : "🔍 Zoom controls disabled.", "info")
    }

    const handleGalleryDownload = (snapshot: Snapshot) => {
        if (snapshot.url) {
            const a = document.createElement("a")
            a.href = snapshot.url
            a.download = `snapshot_${snapshot.id}_${snapshot.date.replace(/-/g, "")}.jpg`
            a.click()
        }
        showToast(`Downloaded: ${snapshot.date}`, "success")
    }
            ...
            <div className="space-y-5 pb-24 px-4 py-5">
                ...
                <div className="bg-gray-900 rounded-2xl aspect-square relative overflow-hidden group shadow-xl">
                    <div className="absolute inset-0 transition-transform duration-300 ease-in-out" style={{ transform: `scale(${zoomLevel})` }}>
                        {!streamError ? (
                            <img ref={videoRef} src={VIDEO_STREAM_URL} alt="Live Kale Tower Feed"
                ...
                <div className="bg-white rounded-2xl p-4 shadow-lg border border-gray-100">
                    ...
                    <div className="space-y-3">
                        {plantDetections.map((plant, idx) => (
                            <div key={idx} className={`flex items-center justify-between p-3 rounded-xl transition-shadow ${plant.color === "emerald" ? "bg-emerald-50 border border-emerald-200 hover:shadow-md" : "bg-amber-50 border border-amber-200 hover:shadow-md"}`}>
                                <div className="flex items-center gap-3">
                                    <div className={`w-3 h-3 rounded-full shadow-inner ${plant.color === "emerald" ? "bg-emerald-500" : "bg-amber-500"}`}></div>
                ...
                <div className="bg-white rounded-2xl p-4 shadow-lg border border-gray-100">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button onClick={handleSnapshot} disabled={streamError}
                            className={`p-4 rounded-xl font-bold transition-all shadow-sm hover:shadow-md active:scale-[0.98] ${streamError ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-pink-100 hover:bg-pink-200 text-pink-700"}`}>
                            📸 Snapshot
                        </button>
                        <button onClick={() => setShowGallery(true)}
                            ...
                            🖼️ Gallery {gallerySnapshots.length > 0 && <span className="text-xs ml-1">({gallerySnapshots.length})</span>}
                        </button>
                        <button onClick={handleRecord} disabled={streamError}
                            className={`p-4 rounded-xl font-bold transition-all shadow-sm hover:shadow-md active:scale-[0.98] ${streamError ? "bg-gray-200 text-gray-400 cursor-not-allowed" : isRecording ? "bg-red-500 hover:bg-red-600 text-white" : "bg-blue-100 hover:bg-blue-200 text-blue-700"}`}>
                            {isRecording ? (
                        ...
                        <button onClick={handleZoomToggle} disabled={streamError}
                        ...
                        <div className="p-4 space-y-6">
                            ...
                            <div className="space-y-3 pt-2">
                                <button onClick={handleSaveSettings} disabled={!hasChanges}
                                    className={`w-full p-4 font-bold rounded-xl transition-colors shadow-lg active:scale-[0.99] ${hasChanges ? "bg-emerald-500 hover:bg-emerald-600 text-white" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}>
                        ...
                        <div className="sticky top-0 bg-white border-b border-gray-200 p-4 flex items-center justify-between z-10 rounded-t-2xl">
                            <div>
                                ...
                                <p className="text-sm text-gray-500">
                                    {snapshotsLoading ? "Loading..." : `${gallerySnapshots.length} snapshots from Raspberry Pi`}
                                </p>
                            </div>
                    ...
                    <div className="max-w-md w-full h-full max-h-[95vh] flex flex-col">
                        ...
                        <div className="bg-white rounded-2xl shadow-2xl overflow-y-auto flex-grow min-h-0">
                            ...
                            <div className="p-6 bg-white">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <button onClick={() => handleGalleryDownload(selectedSnapshot)}
                                        className="p-4 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl flex items-center justify-center gap-2 active:scale-[0.98]">
                                        <Download className="w-5 h-5" />Download
                                    </button>
                                    <button onClick={handleDelete}
                                        className="p-4 bg-red-500 hover:bg-red-600 text-white font-bold rounded-xl flex items-center justify-center gap-2 active:scale-[0.98]">
                                        <Trash2 className="w-5 h-5" />Delete
                                    </button>
                                </div>
                                <button onClick={() => { setSelectedSnapshot(null); setShowGallery(false) }}
